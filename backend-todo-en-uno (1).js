...
const FUENTES = [
  ...
  },
  {
    ...
    horaSorteoRD: { 1: hm(20,55), 2: hm(20,55), 3: hm(20,55), 4: hm(20,55), 5: hm(20,55), 6: hm(20,55) },
  },
  {
    loteria: 'Leidsa',
    juego: 'Pega 3 Más',
...
async function ejecutarScrapingYGuardar() {
  ...
  //
  // CORRECCIÓN: entre 7pm y 11pm RD es cuando MÁS juegos vencen casi juntos
  // (Loteka ~7:55, Leidsa ~8:55, varias franjas de Anguila) — en esa ventana
  // se manda muchas más peticiones totales en poco tiempo que en cualquier
  // otro momento del día, y varios juegos empezaron a salir con datos raros
  // (números que no coinciden ni con hoy ni con ayer) justo en esa franja —
  // posible señal de que la fuente empieza a degradar sus respuestas bajo
  // más carga sostenida. Se le da más espacio entre peticiones en esa
  // ventana específica, sin afectar la velocidad del resto del día.
  const horaActualParaRitmo = horaActualRD();
  const enVentanaNocturnaOcupada = horaActualParaRitmo >= 19 && horaActualParaRitmo <= 23;
  const TAMANO_TANDA = enVentanaNocturnaOcupada ? 1 : 2;
  const DESFASE_MS = enVentanaNocturnaOcupada ? 1500 : 800;
