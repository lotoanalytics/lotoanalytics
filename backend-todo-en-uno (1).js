...
// CORRECCIÓN CRÍTICA (encontrada el 22-08-2026 con HTML de depuración real):
// muchos "no se encontraron números" NO eran bugs de selectores ni de fecha
// — eran Cloudflare (el sistema de seguridad de loteriasdominicanas.com)
// bloqueando la petición y devolviendo una página de "Sorry, you have been
// blocked" en vez del resultado real. Las IPs de GitHub Actions son
// compartidas y conocidas, así que Cloudflare las marca como sospechosas más
// seguido que una IP normal de casa. Por eso pasaba medio al azar: unos
// juegos sí pasaban, otros no, sin ningún patrón relacionado con el código.
//
// La solución: puppeteer-extra + su plugin "stealth", que disimula las
// huellas típicas que Cloudflare usa para detectar navegadores automatizados
// (como navigator.webdriver, el user-agent de headless Chrome, ciertos
// plugins/propiedades que un navegador real sí tiene y uno automatizado no,
// etc.) — no es magia ni garantiza el 100%, pero reduce muchísimo cuántas
// peticiones caen en el bloqueo.
//
// CORRECCIÓN 2 (el mismo día, un rato después): la primera versión de esto
// hacía `require('puppeteer-extra')` directo, sin protección — y si por
// cualquier motivo esa librería no estaba instalada bien (falló el npm
// install, problema de caché, lo que sea), el require tiraba
// MODULE_NOT_FOUND y **tumbaba el script completo antes de revisar UNA SOLA
// lotería** — por eso esa corrida no publicó nada de nada, no fue Cloudflare
// esa vez, fue esto. Ahora el require va dentro de un try/catch: si
// puppeteer-extra/stealth no se puede cargar por lo que sea, el robot cae
// automáticamente a Puppeteer normal (sin el disimulo extra, pero
// funcionando) en vez de morirse entero. Una mejora nunca debe poder tumbar
...
try {
  ...
  console.log('[robot] puppeteer-extra + stealth cargado correctamente — usando disimulo anti-bot.');
} catch (err) {
  console.warn('[robot] No se pudo cargar puppeteer-extra/stealth (' + err.message + ') — se sigue con Puppeteer normal, sin disimulo anti-bot. Revisa que "npm install" haya instalado puppeteer-extra y puppeteer-extra-plugin-stealth.');
}
...
//
// hm(hora, minutos): helper para dar precisión de MINUTO a horaSorteoRD, no
// solo de hora completa. Antes, un juego que sortea a las 7:55 PM solo podía
// escribirse como "19" (7pm) o "20" (8pm) — con "19" la ventana se abría
// desde las 7:00 en punto, 55 minutos antes de que la fuente publicara de
// verdad, y en ese hueco el robot podía guardar el número de AYER con fecha
// de HOY (pasó con Quiniela Loteka, Toca 3, Mega Chances Repartidera — juegos
// de pocos números donde además no aplica la protección de "fantasmas", que
// solo cubre juegos de 4+ números). Con hm(19,55) el corte queda justo en
// 7:55pm, no en 7:00pm.
function hm(hora, minutos) {
  return hora + (minutos || 0) / 60;
}
...
const FUENTES = [
  {
    ...
    horaSorteoRD: hm(14, 30), // CONFIRMADO por el usuario: 2:30 PM
    diasSorteoRD: [1, 2, 3, 4, 5, 6], // CONFIRMADO por el usuario: NO sortea los domingos
  },
  {
    loteria: 'Lotería Nacional',
    juego: 'Juega + Pega +',
    ...
    horaSorteoRD: hm(14, 30), // CONFIRMADO por el usuario: 2:30 PM, diario (incluye domingo)
  },
  {
    loteria: 'Lotería Nacional',
    ...
    // CORRECCIÓN (confirmado por el usuario): el domingo sortea a las 4:00 PM,
    // no a las 6:00 PM como estaba antes.
    horaSorteoRD: { 0: hm(16,0), 1: 21, 2: 21, 3: 21, 4: 21, 5: 21, 6: 21 },
  },
  {
    loteria: 'Leidsa',
    juego: 'Súper Kino TV',
    ...
    // CORREGIDO por el usuario: sortea 9:00 PM todos los días (incluyendo
    // sábado, que antes estaba mal puesto a las 3pm), excepto el domingo que
    // sortea a las 4:00 PM.
    horaSorteoRD: { 0: hm(16,0), 1: 21, 2: 21, 3: 21, 4: 21, 5: 21, 6: 21 },
  },
  {
    loteria: 'Loteka',
    ...
    horaSorteoRD: 20, // CORREGIDO por el usuario a 8:00 PM (antes 7:55pm, confirmado empíricamente el 11-08-2026 — diferencia mínima, sigue dentro del margen de la ventana de horario)
  },
  {
    ...
    horaSorteoRD: 13, // CONFIRMADO por el usuario: 1:00 PM
  },
  {
    loteria: 'Lotería Real',
    juego: 'Tu Fecha Real',
  ...
  },
  {
    ...
    horaSorteoRD: 13, // CONFIRMADO por el usuario
  },
  {
    loteria: 'Lotería Real',
  ...
  },
  {
    ...
    horaSorteoRD: hm(12, 30), // CONFIRMADO por el usuario
  },
  {
    loteria: 'La Suerte Dominicana',
    ...
    horaSorteoRD: 18, // CONFIRMADO por el usuario
  },
  {
    loteria: 'LoteDom',
    ...
    // CORREGIDO por el usuario: sortea 12:00 PM (el comentario viejo decía
    // media tarde ~1:55-2:55 PM, pero la hora real confirmada es mediodía).
    horaSorteoRD: 12,
  },
  {
  ...
  },
  {
    ...
    horaSorteoRD: hm(19, 30), // CORRECCIÓN: sortea 7:30 PM — precisión de minuto
  },
  ...
  // CORRECCIÓN: antes estas 10 franjas diurnas no tenían horaSorteoRD propio,
  // así que solo dependían del corte genérico de las 7am — cualquier corrida
  // después de esa hora aceptaba como "hoy" cualquier número encontrado, sin
  // importar si la franja específica (ej. la 1pm o la 6pm) ya había sorteado
  // de verdad. Eso hacía que a las 8am ya aparecieran "resultados" de la 1pm o
  // 6pm que en la vida real todavía no habían salido. Ahora cada franja tiene
  // su propia hora, igual que ya tenían las nocturnas (8pm/9pm/10pm).
  {
  ...
  },
  {
    ...
    horaSorteoRD: 12, // CONFIRMADO por el usuario: sortea 12:00 PM
  },
  {
    loteria: 'La Primera',
    ...
    // CORRECCIÓN (confirmado en vivo el 5-ago-2026 ~7:08pm RD: el sorteo ya
    // estaba publicado): sortea en la hora 19 (7pm), no 20 — quedó mal puesto
    // en la "corrección" anterior de 21→20, debía ser 19.
    horaSorteoRD: 19,
  },
  {
    loteria: 'La Primera',
    ...
    // CORRECCIÓN (confirmado en vivo el 5-ago-2026 ~7:08pm RD: el sorteo ya
    // estaba publicado): sortea en la hora 19 (7pm), no 20.
    horaSorteoRD: 19,
  },
  {
    loteria: 'La Primera',
    ...
    horaSorteoRD: 12, // CONFIRMADO por el usuario
  },
  {
    loteria: 'La Primera',
    juego: 'El Quinielón Noche',
    bolos: 1,
    // Sorteo propio e independiente: 1 número (00-99), tómbola aparte.
  ...
  },
  {
    ...
    // CORREGIDO por el usuario: 9:00 PM todos los días, excepto domingo 4:00 PM.
    horaSorteoRD: { 0: hm(16,0), 1: 21, 2: 21, 3: 21, 4: 21, 5: 21, 6: 21 },
  },
  {
    loteria: 'Leidsa',
  ...
  },
  {
    ...
    horaSorteoRD: 20, // CORREGIDO por el usuario a 8:00 PM
  },
  {
    loteria: 'Loteka',
  ...
  },
  {
    ...
    // CONFIRMADO: sortea lunes y jueves (junto con el resto de Loteka).
    diasSorteoRD: [1, 4],
    horaSorteoRD: 20, // CORREGIDO por el usuario a 8:00 PM
  },
  {
  ...
  },
  {
    ...
    // CORRECCIÓN 2: se había arreglado a 22 (10:00pm), pero el sorteo real es
    // a las 10:30pm — ese hueco de 30 min hacía que, si el robot corría entre
    // las 10:00 y las 10:30, aceptara el número de AYER como si fuera de hoy,
    // todas las noches — por eso toda la cadena de fechas quedaba corrida un
    // día. Ahora con precisión de minutos.
    horaSorteoRD: hm(22, 30), // NY Numbers Evening, ~10:30 PM hora del este
  },
  {
    loteria: 'Florida',
    ...
    horaSorteoRD: hm(13, 30), // CONFIRMADO por el usuario: 1:30 PM
  },
  {
    loteria: 'Florida',
  ...
  },
  {
    ...
    horaSorteoRD: hm(19, 30), // CORRECCIÓN: 7:30 PM — precisión de minuto
  },
  {
    loteria: 'King Lottery',
    juego: 'Pick 4 Día',
  ...
  },
  {
    ...
    horaSorteoRD: hm(19, 35), // CORRECCIÓN: 7:30 PM, un poco después de Pick 3/4 Noche — precisión de minuto
  },
];
...
// IMPORTANTE — columna nueva en Supabase: esta versión guarda también si el
// sorteo está completo o no (completo:false = todavía le faltan números por
// publicarse). Antes de desplegar este archivo, corre una vez en el SQL
// Editor de Supabase:
//   ALTER TABLE sorteos ADD COLUMN IF NOT EXISTS completo boolean DEFAULT true;
// Con DEFAULT true, todo lo que ya estaba guardado (que siempre fue completo,
// porque antes nunca se guardaba nada parcial) queda marcado correctamente
// sin tener que tocar filas viejas a mano.
async function guardarSorteo(sorteo) {
  ...
  const { data: anteriorCrudo } = await supabase
    .from('sorteos')
    .select('numeros, completo')
    .eq('loteria', sorteo.loteria)
    .eq('juego', sorteo.juego)
    .eq('fecha', sorteo.fecha)
    .maybeSingle();
  const anterior = anteriorCrudo ? { numeros: parsearNumerosGuardados(anteriorCrudo.numeros), completo: anteriorCrudo.completo } : null;
  if (anterior && anterior.completo && anterior.numeros.length >= sorteo.numeros.length) {
    // Ya había un resultado COMPLETO guardado (o al menos con igual o más
    // números) — nunca se sobrescribe un sorteo ya confirmado con un parcial
    // más chico, así varias corridas/reintentos solapados no se pisan entre sí.
    console.warn('[robot] Se descarta lectura de ' + sorteo.loteria + ' — ' + sorteo.juego + ' (' + sorteo.fecha + '): ya había ' + anterior.numeros.length + ' números completos guardados.');
    return { loteria: sorteo.loteria, juego: sorteo.juego, fecha: sorteo.fecha, numeros: anterior.numeros, completo: anterior.completo, descartado: true };
  }
  if (anterior && !anterior.completo && anterior.numeros.length > sorteo.numeros.length) {
    // Había un parcial guardado, pero MÁS avanzado que este — no retrocedas.
    console.warn('[robot] Se descarta lectura de ' + sorteo.loteria + ' — ' + sorteo.juego + ' (' + sorteo.fecha + '): trajo ' + sorteo.numeros.length + ' números, pero ya había un parcial de ' + anterior.numeros.length + ' guardado.');
    return { loteria: sorteo.loteria, juego: sorteo.juego, fecha: sorteo.fecha, numeros: anterior.numeros, completo: anterior.completo, descartado: true };
  }
  const registro = {
    ...
    completo: sorteo.completo !== false, // si no se especifica, se asume completo (mantiene compatibilidad)
  };
...
function normalizarFecha(textoFecha, horaSorteoRD, diasSorteoRD) {
  ...
  if (resultado === fechaHoyRD()) {
    ...
    // Comparación con precisión de MINUTO (no de hora completa) — ver hm() más
    // arriba: así un juego que sortea a las 7:55pm (hm(19,55)) no se abre desde
    // las 7:00pm en punto, evitando aceptar como "de hoy" un número que en
    // realidad sigue siendo el de ayer sin refrescar todavía.
    const minutosActualesRD = minutosDelDiaActualRD();
    const minutosMinimosRD = Math.round(horaMinimaRD * 60);
    if (minutosActualesRD < minutosMinimosRD) {
      console.warn(`[robot] Fecha "${textoFecha}" marcada como hoy (${resultado}) pero son las ${horaActualRD()}:xx RD (este juego sortea a partir de las ${Math.floor(minutosMinimosRD/60)}:${String(minutosMinimosRD%60).padStart(2,'0')}, todavía no ha salido el sorteo real de hoy); se interpreta como el sorteo de anoche y se guarda como ${fechaAyerRD()}.`);
      return fechaAyerRD();
    }
  }
  return resultado;
...
async function normalizarFechaConVerificacion(textoFecha, loteria, juego, numeros, horaSorteoRD, diasSorteoRD) {
  ...
  // NUEVO — extiende la misma protección a juegos CHICOS (2-3 números:
  // Quiniela Nacional, Pega 3 Más, Súper Palé, Quiniela Loteka, Mega Chances
  // Repartidera, Toca 3), que hasta ahora se quedaban SOLO con el corte de
  // horaSorteoRD. Ese corte únicamente cubre "todavía no ha pasado la hora
  // → seguro es de ayer" — si la fuente sigue lenta un rato DESPUÉS de esa
  // hora, no había nada que lo detectara (a diferencia de Súper Kino TV, que
  // por tener 20 números sí cae en el umbral de arriba y queda cubierto).
  //
  // No se puede activar esta comparación TODO el día para juegos chicos (por
  // eso el umbral de 4+ existía): en 2-3 números, un valor repetido de un día
  // para otro puede pasar por pura casualidad, y borrar un resultado real
  // pensando que es fantasma sería peor que el bug que se está arreglando.
  //
  // CORRECCIÓN sobre el primer intento: aquí se había puesto una ventana FIJA
  // de 90 min después de horaSorteoRD (ej. 7:55pm-9:25pm). Falló para
  // Quiniela Loteka, Mega Chances Repartidera y Toca 3 porque el robot corre
  // después de las 10:30pm — para cuando de verdad scrapea, la ventana ya
  // llevaba rato cerrada, así que nunca se activaba. El reloj asumido en
  // horaSorteoRD y la hora real en que corre el robot no son la misma cosa, y
  // depender de que coincidan es fràgil.
  //
  // La señal correcta no es "¿qué hora es?", es "¿ya existe una fila para HOY
  // en este juego?". Si no existe todavía (es la primera vez que se intenta
  // guardar el día de hoy para este juego) y ya pasó su hora de sorteo, y los
  // números son iguales a los de ayer — es fantasma, sin importar a qué hora
  // exacta corrió el robot. Una vez que ya existe una fila real para hoy,
  // este chequeo no se vuelve a activar en el resto del día, así que un
  // resultado que legítimamente se repite más tarde queda protegido igual.
  // CORRECCIÓN IMPORTANTE: la primera versión de este bloque aplicaba a
  // CUALQUIER juego chico que tuviera horaSorteoRD — eso incluía sin querer
  // a Anguila (15 horarios distintos), King Lottery Noche, La Primera Noche,
  // El Quinielón, Súper Palé de Lotería Real/LoteDom, etc. — juegos que YA
  // estaban funcionando bien y que ahora corrían el riesgo de que un número
  // repetido por pura casualidad se borrara pensando que era fantasma (el
  // mismo problema que el umbral de 4+ number existía para evitar). Ahora se
  // restringe a una lista explícita, solo los juegos donde de verdad se
  // confirmó el patrón — igual que JUEGOS_EN_DIAGNOSTICO más abajo en este
  // archivo.
  const JUEGOS_CHICOS_CON_FANTASMA_CONFIRMADO = new Set([
  ...
  let aplicaChequeoFantasma = numeros.length >= UMBRAL_MINIMO_PARA_SOSPECHAR;
  let motivoChequeo = `y son ${numeros.length} números (muy poco probable por azar)`;
  if (!aplicaChequeoFantasma
      && typeof horaSorteoRD !== 'undefined'
      && JUEGOS_CHICOS_CON_FANTASMA_CONFIRMADO.has(loteria + '|' + juego)) {
    const horaResuelta = resolverHoraSorteoRD(horaSorteoRD, fechaHoyRD());
    if (typeof horaResuelta === 'number') {
      const minutosMinimosRD = Math.round(horaResuelta * 60);
      const minutosActualesRD = minutosDelDiaActualRD();
      const yaPasoLaHoraDeSorteo = minutosActualesRD >= minutosMinimosRD;
      if (yaPasoLaHoraDeSorteo) {
        try {
          const { data: filaHoy } = await supabase
            ...
            .maybeSingle();
          if (!filaHoy) {
            aplicaChequeoFantasma = true;
            motivoChequeo = `y todavía no había ninguna fila guardada para hoy en este juego (primera escritura del día, ya pasada su hora de sorteo — sin importar qué hora sea ahora mismo)`;
          }
        } catch (eExiste) {
          console.warn(`[robot] No se pudo verificar si ya existía fila de hoy para ${loteria} — ${juego}: ${eExiste.message}`);
        }
      }
    }
  }
  ...
  if (aplicaChequeoFantasma) {
    try {
      ...
      if (anterior && mismosNumeros(parsearNumerosGuardados(anterior.numeros), numeros)) {
        console.warn(`[robot] ${loteria} — ${juego}: números idénticos al sorteo anterior guardado (${anterior.fecha}), ${motivoChequeo} — se interpreta como fantasma, se mantiene ${anterior.fecha} en vez de ${fechaCalculada}.`);
...
async function obtenerHtmlRenderizado(browser, url) {
  ...
  try {
    ...
    // CORRECCIÓN: la corrida completa se estaba cayendo con "TypeError:
    // Cannot read properties of null (reading 'logger')" — un fallo interno
    // de Puppeteer que pasa cuando llega una solicitud de red justo cuando la
    // página ya navegó o se cerró (el frame quedó desconectado, y req.abort()
    // / req.continue() truena en vez de fallar en silencio). Antes esto no
    // estaba protegido, así que UN solo evento de red residual mataba TODO el
    // proceso de Node — ningún juego de esa corrida se guardaba, sin importar
    // si no tenía nada que ver con la solicitud que falló. Con try/catch, se
    // ignora esa solicitud puntual y la corrida sigue normal.
    page.on('request', (req) => {
      try {
        ...
        console.warn('[scraper] Solicitud de red ignorada (página ya cerrada/navegada): ' + errIntercept.message);
      }
    });
    ...
    //
    // CORRECCIÓN CRÍTICA (22-08-2026): este timeout estaba en 45000 (45
    // segundos) — heredado de cuando el robot SÍ reintentaba varias veces
    // por dentro. Ahora que cada juego se intenta una sola vez y rápido (ver
    // extraerConReintentos), un timeout de 45s por página es carísimo: si
    // una página se cuelga o Cloudflare tarda en responder, esos 45s se
    // comen enteros, y con varios juegos compartiendo la misma hora (batches
    // de 2), esas esperas se ACUMULAN — así es como una corrida terminaba en
    // 3, 4, hasta 5 minutos en vez de los 30-40 segundos que debía tardar.
    // Bajado a 12s: de sobra para una carga normal, pero ya no dejar que UNA
    // página lenta arrastre toda la corrida a varios minutos.
...
// CONFIRMADO EL 11-08-2026 contra la fuente oficial (loteka.com.do): Quiniela
// Loteka, Toca 3 y Mega Chances Repartidera pueden tardar MÁS DE 2 HORAS
// después de su hora de sorteo (7:55pm) en que loteriasdominicanas.com
// publique el valor real — antes de eso, la página muestra temporalmente el
// resultado de AYER bajo la fecha de HOY. El problema es que, en cuanto el
// robot guarda esa fila (completa, con la cantidad correcta de números, solo
// que con el valor viejo), yaTieneResultadoVigente() la da por buena para el
// resto del día y nunca la vuelve a revisar — así que el valor viejo se queda
// para siempre aunque la fuente sí se hubiera corregido después.
// CONFIRMADO EL 18-08-2026: Quiniela Leidsa / Palé mostró el mismo patrón
// (fecha de hoy actualizada, pero número real todavía el de ayer) — se agrega
// a esta misma lista por el mismo motivo que los de Loteka.
const JUEGOS_LENTOS_EN_PUBLICAR = new Set([
  'Loteka|Quiniela Loteka',
  'Loteka|Toca 3',
  'Loteka|Mega Chances Repartidera',
  'Leidsa|Quiniela Leidsa / Palé',
]);
// 3 horas de margen (180 min) — de sobra sobre los ~2h19min que se observó
// que tardó la fuente en corregirse el 11-08-2026.
const MARGEN_REVERIFICACION_MIN = 180;

// ---- Ventana de horario: el robot solo intenta lo que le toca AHORA ----
// Antes, cada corrida repasaba TODOS los ~90 juegos del catálogo y le entraba
// a cualquiera que no tuviera todavía el resultado de hoy — así que un juego
// de las 9pm revisado a las 3pm también entraba a la cola de scraping (sin
// nada que encontrar todavía) y se comía tiempo/reintentos por gusto.
//
// Ahora, antes de intentar nada, se pregunta: "¿ya le tocó su hora a ESTE
// juego en particular (con un poquito de margen antes, por si la fuente
// publica temprano)?" Si todavía no le toca, ni se intenta.
//
// CORRECCIÓN IMPORTANTE (encontrado en producción el 22-08-2026): esta
// ventana ANTES también tenía un límite de "hasta X minutos DESPUÉS de su
// hora" — pasado ese margen, el juego se daba por abandonado hasta el día
// siguiente. El problema real: si la fuente se demora más de esa X (pasa,
// sobre todo con La Primera y Lotería Real, que a veces tardan horas), el
// juego quedaba SIN publicar por el resto del día completo, mostrando el
// resultado de ayer todo ese tiempo — mucho peor que gastar unos segundos
// de más revisándolo. Por eso ahora NO hay límite de "después": una vez que
// le toca su hora, se sigue intentando en TODAS las corridas siguientes,
// sin importar cuánto se demore la fuente, hasta que aparezca — el único
// freno es yaTieneResultadoVigente(), que deja de intentarlo en el momento
// exacto en que ya se consiguió el resultado completo de hoy.
const MARGEN_ANTES_MIN = 10; // hasta 10 min ANTES de su hora (por si la fuente publica temprano)

function estaEnVentanaDeSorteo(fuente) {
  const horaResuelta = resolverHoraSorteoRD(fuente.horaSorteoRD, fechaHoyRD());
  // Juegos sin hora fija conocida (horaSorteoRD sin definir): no se puede
  // calcular ventana, así que no se filtran — se comportan como siempre
  // (protegidos por el corte genérico de las 7am que ya existía).
  if (typeof horaResuelta !== 'number') return true;
  const minutosSorteo = Math.round(horaResuelta * 60);
  const minutosAhora = minutosDelDiaActualRD();
  return minutosAhora >= (minutosSorteo - MARGEN_ANTES_MIN);
}
...
async function yaTieneResultadoVigente(fuente) {
  ...
  try {
    ...
    // NUEVO: para los juegos confirmados lentos en publicar, no basta con que
    // la fila esté completa — dentro de las primeras 3 horas después de su
    // hora de sorteo, SIEMPRE se vuelve a scrapear, para darle campo a la
    // fuente a corregirse si lo que se guardó antes todavía era el valor
    // viejo. Pasadas esas 3 horas, se confía en el atajo normal.
    if (JUEGOS_LENTOS_EN_PUBLICAR.has(fuente.loteria + '|' + fuente.juego) && typeof fuente.horaSorteoRD !== 'undefined') {
      const horaResuelta = resolverHoraSorteoRD(fuente.horaSorteoRD, fechaHoyRD());
      if (typeof horaResuelta === 'number') {
        const minutosMinimosRD = Math.round(horaResuelta * 60);
        const minutosActualesRD = minutosDelDiaActualRD();
        const dentroDelMargen = minutosActualesRD >= minutosMinimosRD && minutosActualesRD <= (minutosMinimosRD + MARGEN_REVERIFICACION_MIN);
        if (dentroDelMargen) {
          console.warn('[robot] ' + fuente.loteria + ' — ' + fuente.juego + ': dentro del margen de reverificación (' + (minutosActualesRD - minutosMinimosRD) + '/' + MARGEN_REVERIFICACION_MIN + ' min desde su sorteo) — se vuelve a scrapear aunque ya haya fila completa, por si la fuente se había corregido.');
          return false;
        }
      }
    }
...
async function extraerUnaFuenteConPuppeteer(browser, fuente) {
  ...
  // Detección explícita del bloqueo de Cloudflare — así, si vuelve a pasar,
  // se ve DE UNA en el log ("🛑 BLOQUEADO POR CLOUDFLARE"), sin tener que
  // bajar el HTML de depuración para descubrirlo cada vez.
  if (html.includes('Attention Required! | Cloudflare') || html.includes('You are unable to access') || html.includes('cf-error-details')) {
    console.warn('[scraper] 🛑 BLOQUEADO POR CLOUDFLARE — ' + fuente.loteria + ' — ' + fuente.juego + ': la fuente devolvió la página de bloqueo en vez del resultado. No es un bug de selectores; es protección anti-bot del sitio. Se reintenta en la próxima corrida.');
    return null;
  }

  const { numeros, textoFecha } = extraerNumerosDeHtml(html);

  // DIAGNÓSTICO TEMPORAL: estos juegos han mostrado números que no coinciden
  // ni con el día anterior ni con la fuente real (posible selector agarrando
  // el bloque equivocado de la página, ya que quedan visualmente cerca de
  // otras quinielas). Se guarda el HTML SIEMPRE para ellos, no solo cuando
  // falla, para poder comparar contra la fuente real la próxima vez y
  // encontrar el bug de verdad en vez de seguir adivinando. Quitar esta lista
  // una vez identificado y corregido el problema real.
  const JUEGOS_EN_DIAGNOSTICO = new Set([
    'Loteka|Quiniela Loteka', 'Loteka|Mega Chances Repartidera', 'Loteka|Toca 3',
    'Leidsa|Súper Palé', 'Leidsa|Quiniela Leidsa / Palé', 'Leidsa|Pega 3 Más',
  ]);
  if (JUEGOS_EN_DIAGNOSTICO.has(fuente.loteria + '|' + fuente.juego)) {
    try {
      const nombreArchivo = 'diag-' + fuente.loteria.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + fuente.juego.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + Date.now() + '.html';
      ...
      console.warn('[diagnostico] ' + fuente.loteria + ' — ' + fuente.juego + ': números leídos ' + JSON.stringify(numeros) + ', HTML guardado en ' + rutaDebug);
    } catch (errGuardado) {
      console.warn('[diagnostico] No se pudo guardar HTML de diagnóstico: ' + errGuardado.message);
    }
  }
  ...
  // CORRECCIÓN: si sabemos cuántos números debe traer este juego
  // (fuente.bolos) y encontramos MENOS que eso, es señal de que Puppeteer
  // agarró la página a mitad de actualizarse (ej. 2 de las 3 bolas ya
  // pintadas en pantalla, la tercera todavía no).
  //
  // ANTES: esto se trataba igual que "no se encontró nada" y se descartaba
  // por completo — el robot solo insistía ~3 minutos y si no se completaba
  // en ese margen, se rendía hasta la próxima corrida, aunque ya hubiera
  // números publicados de verdad.
  //
  // AHORA: se devuelve el parcial marcado con completo:false. Quien llama
  // (extraerConReintentos) decide qué hacer con él — normalmente: guardarlo
  // YA en Supabase (para que se vea "en vivo" en la app, bola por bola, como
  // la página de Conéctate) y seguir insistiendo por el resto.
  const completo = !(typeof fuente.bolos === 'number' && numeros.length < fuente.bolos);
  if (!completo) {
    console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': encontrados ' + numeros.length + '/' + fuente.bolos + ' números — parcial, todavía se está actualizando.');
  }
...
// ---- Un solo intento rápido por juego, sin quedarse esperando ----
// ANTES: por cada juego pendiente, el robot se quedaba insistiendo dentro
// de la MISMA corrida (esperando 15s entre intentos, hasta 1 minuto). Con
// varios juegos compartiendo la misma hora (LoteDom, Leidsa, Lotería Real,
// La Primera...), esas esperas se sumaban una detrás de otra y la corrida
// completa terminaba tardando 3, 4, hasta 5 minutos — carísimo en minutos
// de GitHub Actions.
//
// AHORA: cada juego se revisa UNA sola vez por corrida, rápido, sin ninguna
// espera artificial. Si en ese momento ya está publicado, se guarda. Si
// todavía no, no pasa nada malo — la ventana de horario (estaEnVentanaDeSorteo)
// ya NO tiene límite de "hasta tal hora después", así que ese mismo juego
// se vuelve a intentar en la SIGUIENTE corrida, y en la siguiente, y así
// hasta que aparezca — sin importar si tarda 2 minutos o 3 horas. La
// persistencia ya no vive DENTRO de una corrida (cara, lenta), vive ENTRE
// corridas (barata, rápida) — apoyada en que disparas el robot varias veces
// seguidas desde cron-job.org.
async function extraerConReintentos(browser, fuente) {
  let resultado;
  try {
    resultado = await extraerUnaFuenteConPuppeteer(browser, fuente);
  } catch (err) {
    console.error('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': error leyendo la página:', err.message);
    return null;
  }

  if (!resultado) {
    console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': no se encontró nada todavía, se deja para la próxima corrida.');
    return null;
  }
  ...
  // Parcial: se guarda igual (para que se vea "en vivo" en la app) aunque no
  // esté completo — la próxima corrida sigue intentando completarlo.
  if (resultado.numeros.length > 0) {
    console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': parcial (' + resultado.numeros.length + '/' + fuente.bolos + '), se guarda en vivo y se completa en una próxima corrida.');
    return resultado;
  }
...
async function ejecutarScrapingYGuardar() {
  ...
  for (const fuente of fuentesOrdenadas) {
    try {
      if (!estaEnVentanaDeSorteo(fuente)) {
        // Ni siquiera le toca todavía (o ya pasó de sobra su horario) — no se
        // gasta ni una consulta a Supabase en él, mucho menos scraping.
        continue;
      }
  ...
  console.log('[robot] ' + porScrapear.length + ' juego(s) por scrapear de verdad (de ' + fuentesOrdenadas.length + ' en el catálogo, dentro de su ventana de horario ahora mismo).');
  ...
  // Red de seguridad para toda la corrida: ahora que cada juego se revisa
  // una sola vez y rápido (timeout de página bajado a 12s, ver
  // obtenerHtmlRenderizado), esto casi nunca se debería alcanzar — pero si
  // hay MUCHÍSIMOS juegos pendientes a la vez (o la fuente está respondiendo
  // lenta esa noche), esto evita que una corrida se dispare a varios
  // minutos sin control.
  const LIMITE_GLOBAL_CORRIDA_MS = 60 * 1000; // tope de 1 minuto
  const finGlobalCorrida = Date.now() + LIMITE_GLOBAL_CORRIDA_MS;

  try {
    for (let i = 0; i < porScrapear.length; i += TAMANO_TANDA) {
      if (Date.now() >= finGlobalCorrida) {
        console.warn('[robot] Se alcanzó el límite de tiempo de la corrida — quedan ' + (porScrapear.length - i) + ' juego(s) sin revisar en esta pasada, se cubren en la próxima corrida.');
        break;
      }
      ...
      await Promise.all(tanda.map(async (fuente, indiceEnTanda) => {
        ...
        await new Promise((r) => setTimeout(r, indiceEnTanda * DESFASE_MS));
        try {
          const sorteo = await extraerConReintentos(browser, fuente);
          if (sorteo) {
            // Este guardarSorteo() puede ser redundante si extraerConReintentos
            // ya guardó exactamente este mismo resultado en vivo — el upsert es
            // idempotente así que no hace daño, solo confirma el estado final.
            const guardado = await guardarSorteo(sorteo);
            resultados.guardados.push(guardado);
            const estado = sorteo.completo === false ? 'PARCIAL (' + sorteo.numeros.length + '/' + fuente.bolos + ')' : 'COMPLETO';
            console.log('[scraper] ' + estado + ' — Guardado: ' + sorteo.loteria + ' — ' + sorteo.juego + ' (' + sorteo.fecha + ') -> [' + sorteo.numeros.join(', ') + ']');
          }
        } catch (err) {
          resultados.errores.push({ fuente: fuente.loteria + ' — ' + fuente.juego, error: err.message });
