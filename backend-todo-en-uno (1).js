...
async function normalizarFechaConVerificacion(textoFecha, loteria, juego, numeros, horaSorteoRD, diasSorteoRD) {
  // OJO — HISTORIAL IMPORTANTE: esta función antes comparaba los números
  // recién scrapeados contra el sorteo guardado del día anterior, y si
  // coincidían exactamente, asumía que era un "fantasma" (la fuente sin
  // refrescar) y BORRABA la fila de hoy, dejando la de ayer.
  //
  // Se quitó esa lógica porque causaba falsos positivos reales: en juegos
  // con pocos números (El Quinielón: 1 número del 00-99, Súper Palé: 2
  // números, etc.) el mismo número puede salir dos días seguidos por pura
  // casualidad — no es un error, es solo que el sorteo real repitió. Cuando
  // eso pasaba, esta función borraba el resultado LEGÍTIMO de hoy pensando
  // que era un fantasma, dejando pegada la fecha de ayer (y la bolita
  // apagada) aunque el sorteo de hoy sí había salido normal.
  //
  // Ahora la fecha se decide ÚNICAMENTE con horaSorteoRD/diasSorteoRD (en
  // normalizarFecha) — esa protección es más confiable porque se basa en la
  // hora real de cada sorteo, no en adivinar por coincidencia de números.
  return normalizarFecha(textoFecha, horaSorteoRD, diasSorteoRD);
}
...
async function ejecutarScrapingYGuardar() {
  ...
  try {
    // ---- Procesamiento en tandas concurrentes ----
    // Antes cada juego se procesaba uno por uno, de principio a fin — si a las
    // 9pm había que scrapear 15-20 juegos de verdad, el tiempo se sumaba todo
    // en fila (15-20 × varios segundos cada uno). Ahora se arma primero la
    // lista de "cuáles de verdad hay que scrapear" (los chequeos contra
    // Supabase para saltar los que ya están al día son baratos y se hacen
    // rápido, uno detrás de otro), y luego esos pocos que sí necesitan
    // Puppeteer se procesan en TANDAS de varios a la vez — se respeta el orden
    // de urgencia ya calculado (los más urgentes van en las primeras tandas).
    ...
    console.log('[robot] ' + porScrapear.length + ' juego(s) por scrapear de verdad, en tandas de ' + TAMANO_TANDA + '.');

    for (let i = 0; i < porScrapear.length; i += TAMANO_TANDA) {
      const tanda = porScrapear.slice(i, i + TAMANO_TANDA);
      await Promise.all(tanda.map(async (fuente) => {
        try {
