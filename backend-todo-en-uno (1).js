...
async function obtenerHtmlRenderizado(browser, url) {
  ...
  try {
    ...
    // Pequeño margen extra por si algún dato llega en una petición tardía
...
function extraerNumerosDeHtml(html) {
  ...
  // Búsqueda directa por bloque de juego
  ...
  // Si no se halló en bloques, buscar globalmente todas las bolitas numeradas
  if (numeros.length === 0) {
    $('span.score, .score, .bola, .ball').each(function (_, el) {
