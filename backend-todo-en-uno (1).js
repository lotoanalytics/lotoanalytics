...
function normalizarFecha(textoFecha) {
  ...
  // Ningún sorteo dominicano real sale entre medianoche y las 7:00 AM. Si el
  // robot corre en esa ventana (por los reintentos, o por demoras de GitHub
  // Actions) y la fuente YA marca su resultado como "hoy" (su reloj cambió de
  // día a medianoche), es casi seguro que en realidad es el sorteo de ANOCHE —
  // el sorteo real de hoy todavía ni ha empezado. A diferencia de la protección
  // de normalizarFechaConVerificacion (que compara contra el historial ya
  // guardado), esta funciona incluso para un juego que se guarda por primera
  // vez, porque no depende de tener una fila de "ayer" contra la cual comparar.
  if (resultado === fechaHoyRD() && horaActualRD() < 7) {
    console.warn(`[robot] Fecha "${textoFecha}" marcada como hoy (${resultado}) pero son las ${horaActualRD()}:xx RD (antes de las 7am, ningún sorteo real ha salido aún hoy); se interpreta como el sorteo de anoche y se guarda como ${fechaAyerRD()}.`);
    return fechaAyerRD();
  }
  return resultado;
}
...
async function normalizarFechaConVerificacion(textoFecha, loteria, juego, numeros) {
  const fechaCalculada = normalizarFecha(textoFecha);
  ...
  try {
    ...
    if (
      ultimo &&
      Array.isArray(ultimo.numeros) &&
      JSON.stringify(ultimo.numeros) === JSON.stringify(numeros) &&
      ultimo.fecha < fechaCalculada
    ) {
      console.warn(`[robot] ${loteria} — ${juego}: números idénticos al último sorteo ya guardado (${ultimo.fecha}); todavía no hay sorteo nuevo, se mantiene esa fecha en vez de ${fechaCalculada}.`);
      fecha = ultimo.fecha;

      // Si de una corrida anterior (antes de esta protección) ya había quedado
      // una fila fantasma con estos mismos números en la fecha mala, se borra
      // para no dejarla duplicada para siempre.
      try {
    ...
    console.warn(`[robot] No se pudo verificar el último sorteo guardado para ${loteria} — ${juego}: ${e.message}`);
  }
  return fecha;
}
