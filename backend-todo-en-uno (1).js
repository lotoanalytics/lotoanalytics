...
async function normalizarFechaConVerificacion(textoFecha, loteria, juego, numeros, horaSorteoRD, diasSorteoRD) {
  ...
  // La salida: acotarla a una VENTANA CORTA justo después de la hora en que
  // le tocaba sortear a ESE juego — es exactamente el momento donde la fuente
  // publica lenta, y una coincidencia real por casualidad justo en esa
  // ventana es rarísima. Fuera de la ventana, el juego chico se queda como
  // antes, sin este chequeo, protegiendo resultados reales repetidos más
  // tarde en el día.
  const VENTANA_FANTASMA_MIN = 90; // minutos después de horaSorteoRD
  ...
  if (!aplicaChequeoFantasma && typeof horaSorteoRD !== 'undefined') {
    ...
    if (typeof horaResuelta === 'number') {
      ...
      const minutosActualesRD = minutosDelDiaActualRD();
      const dentroDeVentana = minutosActualesRD >= minutosMinimosRD && minutosActualesRD <= (minutosMinimosRD + VENTANA_FANTASMA_MIN);
      if (dentroDeVentana) {
        aplicaChequeoFantasma = true;
        motivoChequeo = `y estamos a ${minutosActualesRD - minutosMinimosRD} min de que le tocó sortear (dentro de la ventana de ${VENTANA_FANTASMA_MIN} min donde es muy probable que la fuente no se haya refrescado todavía)`;
      }
    }
  }
