...
//
// Ahora, antes de intentar nada, se pregunta: "¿está el reloj de RD dentro
// del margen razonable alrededor de la hora en que ESTE juego en particular
// sortea?" Si no, ni se toca — se deja completo para cuando de verdad le
// toque. Esto es justo lo que permite disparar el robot cada pocos minutos
// (2:59, 3:03, 3:05...) desde cron-job.org sin miedo: cada corrida solo
// revisa el puñado de juegos que están cerca de su hora en ESE momento, no
// el catálogo completo.
const MARGEN_ANTES_MIN = 10;   // hasta 10 min ANTES de su hora (por si la fuente publica temprano)
const MARGEN_DESPUES_MIN = 60; // hasta 60 min DESPUÉS de su hora (cubre demoras normales)
...
function estaEnVentanaDeSorteo(fuente) {
  ...
  // A los juegos ya confirmados lentos en corregirse (JUEGOS_LENTOS_EN_PUBLICAR)
  // se les da el mismo margen amplio que ya usa yaTieneResultadoVigente para
  // sus reverificaciones (3 horas), así ambas ventanas coinciden.
  const margenDespues = JUEGOS_LENTOS_EN_PUBLICAR.has(fuente.loteria + '|' + fuente.juego)
    ? MARGEN_REVERIFICACION_MIN
    : MARGEN_DESPUES_MIN;
  return minutosAhora >= (minutosSorteo - MARGEN_ANTES_MIN) && minutosAhora <= (minutosSorteo + margenDespues);
}
...
// ---- Reintento dentro de la MISMA corrida: espera activa "en vivo" ----
...
// había que esperar a la PRÓXIMA corrida completa para agarrarla.
//
// AHORA va más allá: mientras espera, cada vez que aparece un número nuevo
// (aunque el juego todavía no esté completo), lo GUARDA de una vez en
// Supabase. Como la app ya está conectada por Realtime, eso hace que las
// bolitas vayan apareciendo/actualizándose EN EL MOMENTO en la pantalla del
// usuario — igual que en la página de Conéctate — en vez de que todo el
// resultado aparezca de golpe al final. Y como ahora SÍ se está guardando
// progreso real, el robot no se rinde a los 3 minutos como antes: se queda
// revisando cada ESPERA_MS hasta completar el juego o hasta llegar al tope
// de tiempo (TOPE_ESPERA_MS) — lo que pase primero.
// Ahora que estaEnVentanaDeSorteo() ya garantiza que un juego solo entra
// aquí cuando de verdad le toca (o le acaba de tocar), no hace falta que
// UNA sola corrida se quede esperando 15 minutos por él — eso era lo que se
// comía los minutos de GitHub. En vez de eso: cada corrida insiste rápido,
// unos ~60 segundos (4 intentos de 15s), guardando cualquier parcial que
// encuentre en el camino. Si en ese minuto no se completa, la corrida
// termina — y el SIGUIENTE disparo de cron-job.org (2:59, 3:03, 3:05...)
// retoma exactamente donde quedó, porque yaTieneResultadoVigente() ve que
// sigue incompleto y lo vuelve a intentar. Así cada corrida individual es
// corta y barata, y entre varios disparos seguidos se cubre igual la demora
// real de la fuente.
const ESPERA_ACTIVA_MS = 15000; // cada 15s mientras el juego sigue incompleto
const TOPE_ESPERA_ACTIVA_MS = 60 * 1000; // máximo ~1 minuto de espera activa POR JUEGO

// ---- Límite GLOBAL de toda la corrida (esto es lo que faltaba) ----
// El TOPE_ESPERA_ACTIVA_MS de arriba es por JUEGO — pero varios juegos
// comparten la misma hora exacta (los 4 de LoteDom a las 12:00, los 5 de
// Leidsa a las 9pm, los 5 de Lotería Real a la 1pm...). Como se procesan de
// a 2 en 2 (TAMANO_TANDA), si hay 4-5 juegos pendientes a la misma hora,
// eso son 2-3 tandas SEGUIDAS, y cada una podía tardar hasta 1 minuto — la
// corrida completa terminaba tardando 2-3 minutos, aunque cada juego
// individualmente respetara su propio límite. Eso es lo que se veía en las
// corridas de GitHub Actions de 1m30s, 2m29s, etc.
//
// Ahora hay un reloj ÚNICO para toda la corrida (finGlobalCorrida, ver
// ejecutarScrapingYGuardar): todos los juegos, sin importar en qué tanda
// estén, dejan de insistir en el momento en que se cumple ese límite
// compartido — así la corrida completa nunca pasa de este tiempo, sin
// importar cuántos juegos coincidan en la misma hora.
const LIMITE_GLOBAL_CORRIDA_MS = 45 * 1000; // ~45s de scraping activo (deja margen para abrir/cerrar el navegador y quedar bajo 1 min en total)

async function extraerConReintentos(browser, fuente, finGlobalCorrida) {
  const inicio = Date.now();
  const finPropio = inicio + TOPE_ESPERA_ACTIVA_MS;
  // El que gane más pronto manda: el tope propio de este juego, o el reloj
  // compartido de toda la corrida — lo que llegue primero.
  const finReal = typeof finGlobalCorrida === 'number' ? Math.min(finPropio, finGlobalCorrida) : finPropio;
  let mejorParcialGuardado = null;
  let intento = 0;

  while (Date.now() < finReal) {
    intento++;
    let resultado;
    try {
      resultado = await extraerUnaFuenteConPuppeteer(browser, fuente);
    } catch (err) {
      console.error('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': error leyendo la página (intento ' + intento + '):', err.message);
      resultado = null;
    }

    if (resultado && resultado.completo) {
      // ¡Listo! Ya está completo — se guarda (si no se había guardado ya
      // exactamente así) y se acaba la espera para este juego.
      return resultado;
    }

    if (resultado && resultado.numeros.length > 0) {
      // Parcial: se guarda YA si es mejor que lo último que se guardó en
      // esta misma espera, para que se vea "en vivo" sin esperar a que
      // termine. No hace falta esperar el veredicto de guardarSorteo para
      // seguir el ciclo — si falla el guardado, se reintenta en la próxima
      // vuelta de todos modos porque mejorParcialGuardado no se actualiza.
      if (!mejorParcialGuardado || resultado.numeros.length > mejorParcialGuardado.numeros.length) {
        try {
          await guardarSorteo(resultado);
          mejorParcialGuardado = resultado;
          console.log('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': parcial guardado EN VIVO (' + resultado.numeros.length + '/' + fuente.bolos + ') -> [' + resultado.numeros.join(', ') + ']');
        } catch (err) {
          console.warn('[scraper] No se pudo guardar el parcial de ' + fuente.loteria + ' — ' + fuente.juego + ':', err.message);
        }
      }
    }

    const restanteMs = finReal - Date.now();
    if (restanteMs <= 0) break;
    const esperaMs = Math.min(ESPERA_ACTIVA_MS, restanteMs);
    console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': todavía no está completo (intento ' + intento + '), esperando ' + Math.round(esperaMs / 1000) + 's antes de revisar de nuevo...');
    await new Promise((r) => setTimeout(r, esperaMs));
  }

  if (mejorParcialGuardado) {
    console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': se agotó el tiempo disponible sin completarse — se deja el parcial guardado (' + mejorParcialGuardado.numeros.length + '/' + fuente.bolos + '), la próxima corrida sigue de donde quedó.');
    return mejorParcialGuardado;
  }
  console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': se agotó el tiempo disponible sin encontrar nada, se deja para la próxima corrida.');
  return null;
}
...
async function ejecutarScrapingYGuardar() {
  ...
  // Reloj compartido por TODA la corrida — arranca a contar justo ahora,
  // después de abrir el navegador (que ya de por sí toma varios segundos).
  // Todos los juegos, sin importar en qué tanda les toque, dejan de insistir
  // en cuanto se cumpla este límite — ver la explicación completa junto a
  // LIMITE_GLOBAL_CORRIDA_MS más arriba.
  ...
  try {
    for (let i = 0; i < porScrapear.length; i += TAMANO_TANDA) {
      ...
      await Promise.all(tanda.map(async (fuente, indiceEnTanda) => {
        ...
        try {
          const sorteo = await extraerConReintentos(browser, fuente, finGlobalCorrida);
          if (sorteo) {
