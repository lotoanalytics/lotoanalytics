...
const FUENTES = [
  ...
  },
  {
    ...
    // a las 6pm.
    // CORRECCIÓN: antes se dejaba sin entrada para el día 0 (domingo) con el
    // razonamiento de "sale antes de las 8pm, no hace falta restricción" —
    // pero ese razonamiento es incorrecto: sin esa entrada, el código cae al
    // genérico de las 7am, dejando un hueco de 7am a 6pm donde se podía
    // aceptar como "de hoy" un número viejo sin refrescar. Mismo bug real que
    // se encontró y confirmó en el grupo de Leidsa (Súper Palé, etc.) el 9 de
    // agosto — se corrige aquí también con la misma lógica.
  ...
  },
  {
    ...
    // CORRECCIÓN: por el mismo motivo de arriba (Quiniela Nacional), se le
    // agregan las entradas de fin de semana explícitamente en vez de dejarlas
    // vacías — aunque no había dado problema todavía, el hueco de protección
    // era el mismo y más vale cerrarlo antes de que falle.
    horaSorteoRD: { 0: hm(15,0), 1: 21, 2: 21, 3: 21, 4: 21, 5: 21, 6: hm(15,0) },
  },
  {
    loteria: 'Loteka',
    ...
    horaSorteoRD: hm(19, 55), // CORRECCIÓN: sortea ~7:55 PM — precisión de minuto, no solo la hora 19, para no aceptar como "hoy" el número de ayer entre las 7:00 y las 7:55
  },
  {
  ...
  },
  {
    ...
    // lunes a sábado 8:55pm, domingo 3:55pm.
    // CORRECCIÓN: precisión de minutos, antes solo tenía la hora completa 21
    // (9pm), abriendo la ventana 5 min tarde respecto al 8:55pm real.
    // CORRECCIÓN 2: el domingo (0) estaba documentado en el comentario pero
    // nunca se agregó de verdad al objeto — sin esa llave, resolverHoraSorteoRD
    // devolvía undefined y el código caía al genérico de las 7am (sin ninguna
    // protección real), aceptando como "de hoy" cualquier cosa que hubiera en
    // la página desde temprano en la mañana del domingo.
    horaSorteoRD: { 0: hm(15,55), 1: hm(20,55), 2: hm(20,55), 3: hm(20,55), 4: hm(20,55), 5: hm(20,55), 6: hm(20,55) },
  },
  {
    loteria: 'Leidsa',
    juego: 'Pega 3 Más',
    ...
    // Lunes a sábado 8:55pm; domingo 3:55pm.
    // CORRECCIÓN: precisión de minutos, igual que Loto Pool.
    // CORRECCIÓN 2: se agrega la llave del domingo (0), que faltaba — ver nota
    // completa en Loto Pool.
    horaSorteoRD: { 0: hm(15,55), 1: hm(20,55), 2: hm(20,55), 3: hm(20,55), 4: hm(20,55), 5: hm(20,55), 6: hm(20,55) },
  },
  {
    loteria: 'Leidsa',
  ...
  },
  {
    ...
    horaSorteoRD: hm(19, 55), // CORRECCIÓN: ~7:55 PM — precisión de minuto, no toda la hora 19
  },
  {
    loteria: 'Loteka',
    ...
    horaSorteoRD: hm(19, 55), // CORRECCIÓN: depende de Mega Chances (~7:55 PM) — precisión de minuto
  },
  {
    loteria: 'Loteka',
    juego: 'Toca 3',
    ...
    horaSorteoRD: hm(19, 55), // CORRECCIÓN: ~7:55 PM — precisión de minuto
  },
  {
    loteria: 'Loteka',
    juego: 'Mega Lotto',
    ...
    // Loteka). CORRECCIÓN: se había quedado sin horaSorteoRD cuando se le
    // agregó precisión de minutos a los demás juegos de Loteka — dependía
    // solo del corte genérico de 7am, igual que le pasaba a Anguila.
    diasSorteoRD: [1, 4],
    horaSorteoRD: hm(19, 55),
  },
  {
    loteria: 'Lotería Real',
    juego: 'Loto Real',
...
const TOPE_ESPERA_ACTIVA_MS = 60 * 1000; // máximo ~1 minuto de espera activa POR CORRIDA
...
async function extraerConReintentos(browser, fuente) {
  ...
  while (Date.now() - inicio < TOPE_ESPERA_ACTIVA_MS) {
    intento++;
    let resultado;
    try {
    ...
    const restanteMs = TOPE_ESPERA_ACTIVA_MS - (Date.now() - inicio);
  ...
  if (mejorParcialGuardado) {
    console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': se agotó el tope de espera (' + (TOPE_ESPERA_ACTIVA_MS / 60000) + ' min) sin completarse — se deja el parcial guardado (' + mejorParcialGuardado.numeros.length + '/' + fuente.bolos + '), la próxima corrida sigue de donde quedó.');
    return mejorParcialGuardado;
  }
  console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': se agotó el tope de espera (' + (TOPE_ESPERA_ACTIVA_MS / 60000) + ' min) sin encontrar nada, se deja para la próxima corrida.');
  return null;
}
