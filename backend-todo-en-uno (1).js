...
function normalizarFecha(textoFecha) {
  ...
  // Ningún sorteo puede ser "de mañana" — si la página fuente marcó la fecha de
  // hoy como si fuera el día siguiente (les pasa de noche, cuando su servidor
  // calcula la fecha en UTC en vez de hora RD), se corrige aquí en vez de
  // guardar un sorteo con fecha futura.
  if (resultado === fechaMananaRD()) {
    console.warn(`[robot] Fecha "${textoFecha}" venía marcada como mañana (${resultado}); corregida a hoy (${fechaHoyRD()}).`);
    return fechaHoyRD();
  }
  return resultado;
}
