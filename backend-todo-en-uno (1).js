...
const FUENTES = [
  {
    ...
    horaSorteoRD: hm(14, 30), // CONFIRMADO por el usuario: 2:30 PM, todos los días (incluye domingo)
  },
  {
    loteria: 'Lotería Nacional',
    juego: 'Juega + Pega +',
  ...
  },
  {
    ...
    // a las 6:00 PM (CONFIRMADO de nuevo por el usuario — hubo una corrección
    // anterior a las 4:00 PM que resultó estar equivocada).
    horaSorteoRD: { 0: hm(18,0), 1: 21, 2: 21, 3: 21, 4: 21, 5: 21, 6: 21 },
  },
  {
    loteria: 'Leidsa',
    juego: 'Súper Kino TV',
...
// ---- (Quitado 22-08-2026) Filtro de "ventana de horario" ----
// Aquí antes vivía estaEnVentanaDeSorteo(): un filtro que descartaba de
// entrada cualquier juego que no estuviera "cerca" de su horario, con la
// idea de ahorrar tiempo. En la práctica se convirtió en el propio bug —
// varios juegos (LoteDom, entre otros) se quedaban SIN revisar por horas,
// sin ningún error visible, porque este filtro los descartaba antes de
// darles la oportunidad. Se quitó por completo: ahora TODOS los juegos sin
// resultado de hoy se revisan siempre, priorizados por urgencia
// (ordenarPorUrgencia, más abajo) — el que le tocó sortear más reciente va
// primero, pero ninguno se descarta de plano. La velocidad ya no depende de
// filtrar de más, depende de que cada intento sea rápido (un solo intento,
// 12s de margen por página — ver obtenerHtmlRenderizado) y de que
// yaTieneResultadoVigente() salte gratis, sin abrir el navegador, los que
// ya están al día.
...
// todavía no, no pasa nada malo — ya no hay ningún filtro de horario que
// pueda dejarlo abandonado (ver la nota junto a MARGEN_REVERIFICACION_MIN),
// así que ese mismo juego se vuelve a intentar en la SIGUIENTE corrida, y en
// la siguiente, y así hasta que aparezca — sin importar si tarda 2 minutos o
// 3 horas. La persistencia ya no vive DENTRO de una corrida (cara, lenta),
// vive ENTRE corridas (barata, rápida) — apoyada en que disparas el robot
// varias veces seguidas desde cron-job.org.
...
async function ejecutarScrapingYGuardar() {
  ...
  // Puppeteer) cuáles juegos de verdad necesitan scrapearse en este
  // momento. Si no hay ninguno, la corrida termina en segundos SIN haber
  // abierto el navegador — casi gratis en minutos de Actions.
  //
  // CORRECCIÓN (22-08-2026): antes, ANTES de siquiera revisar Supabase,
  // había un filtro extra (estaEnVentanaDeSorteo) que descartaba de una vez
  // cualquier juego que no estuviera "cerca" de su horario — la idea era
  // ahorrar tiempo, pero en la práctica terminó siendo el propio bug: varios
  // juegos (los de LoteDom, por ejemplo) se quedaban SIN revisar por horas,
  // sin ningún error ni explicación, porque ese filtro los descartaba antes
  // de darles la oportunidad. Se quitó por completo. Ahora se revisan TODOS
  // los juegos que no tengan resultado de hoy — el orden por urgencia
  // (ordenarPorUrgencia, arriba) ya se encarga de que el que le tocó más
  // recientemente se intente PRIMERO, pero ninguno se descarta de plano.
  // Como cada intento es rápido (una sola vez, 12s de margen por página) y
  // yaTieneResultadoVigente() ya salta gratis los que están al día, esto
  // sigue siendo rápido sin dejar a nadie abandonado.
  const porScrapear = [];
  ...
  console.log('[robot] ' + porScrapear.length + ' juego(s) por scrapear de verdad (de ' + fuentesOrdenadas.length + ' en el catálogo), en orden de urgencia.');
