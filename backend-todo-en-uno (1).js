...
async function extraerUnaFuenteConPuppeteer(browser, fuente) {
  ...
  // pintadas en pantalla, la tercera todavía no). Antes esto se aceptaba
  // como si fuera un resultado completo y se guardaba a medias para
  // siempre — ahora se trata igual que "no se encontró nada", así entra al
  // mismo ciclo de reintentos (extraerConReintentos) hasta conseguir el
  // conjunto completo, en vez de quedarse pegado con un resultado parcial.
  if (typeof fuente.bolos === 'number' && numeros.length < fuente.bolos) {
    console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': encontrados ' + numeros.length + '/' + fuente.bolos + ' números — parece que la página todavía se está actualizando, se trata como no publicado todavía.');
    return null;
  }
...
async function extraerConReintentos(browser, fuente) {
  // Antes: 3 intentos cada 45s (90s de margen total). Eso alcanzaba solo si
  // la fuente publicaba casi al instante — pero loteriasdominicanas.com a
  // veces tarda 2-4 minutos en actualizar la página después de la hora
  // exacta del sorteo. Con 90s de margen, esos casos se perdían y el juego
  // se quedaba pendiente hasta la próxima corrida completa (que puede tardar
  // varios minutos más en dispararse, según cron-job.org).
  //
  // Ahora: 6 intentos con espera creciente (20s, 20s, 30s, 30s, 40s, 40s),
  // dando ~3 minutos de margen total — cubre la demora típica de la fuente
  // SIN afectar la velocidad de las corridas normales, porque este reintento
  // solo se activa cuando de verdad hay un juego pendiente de publicar (el
  // chequeo rápido contra Supabase, antes de abrir el navegador, sigue
  // saltándose por completo los juegos que ya están al día).
  const ESPERAS_MS = [20000, 20000, 30000, 30000, 40000, 40000];
  const INTENTOS_MAXIMOS = ESPERAS_MS.length + 1;
  for (let intento = 1; intento <= INTENTOS_MAXIMOS; intento++) {
    ...
    if (intento < INTENTOS_MAXIMOS) {
      ...
      console.warn('[scraper] ' + fuente.loteria + ' — ' + fuente.juego + ': todavía no está publicado (intento ' + intento + '/' + INTENTOS_MAXIMOS + '), esperando ' + (esperaMs / 1000) + 's antes de reintentar...');
